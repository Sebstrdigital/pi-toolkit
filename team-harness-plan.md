...
5. **Dan's skill/expertise/domain primitives** (from his `vue-generator.md` screenshots, 2026-04-27). Three concepts to evaluate later:
   - ✅ **`skills:` per-agent** — done in commit `24923de` (active-listener + precise-worker shipped).
   ...
   - **`domain:` paths — real ACL enforcement** — current implementation injects scope as a prompt-level hard rule, but small models ignore it (verified 2026-04-27 stress test: ling-2.6-flash-free wrote outside `src/**`). To enforce, either intercept Edit calls inside pi-team and gate on glob match, or run the worker subprocess in a chroot/container. Defer until frontier models are on or stress test demands it.

6. **Persistent agent processes** (deferred from 2026-04-27). Today every `mention(...)` calls `spawn("pi", ...)` — fresh OS subprocess per turn, ~1–3s startup overhead each. Refactor `agent-process.ts` to keep one `ChildProcess` alive per role for the duration of the harness session: send turn prompts on stdin, parse JSON events from stdout indefinitely. Adds ~14–40s back to a 7-task run. Hard part: handling pi's context-window-full case (need to detect, summarize/restart that one process without tearing down the team). Worth doing once we hit longer runs where startup cost dominates.

7. **Chat UI redesign** (deferred from 2026-04-27). User wants Dan-style per-agent message rendering: colored badge with tier icon (`■ @ui-lead 14:22`) on a header line and a colored `│` gutter down the left of the message body, instead of pi's default tool-result block. Two stages:
   - **Lightweight** (interim, ~30 min): prefix each section in `renderTranscript` with a colored badge line and indent the body with a `│` gutter. Stays inside pi's tool-result block, but visually grouped per agent. Acceptable until the heavy version is built.
   - **Heavy** (~days): bypass pi's chat renderer for harness messages — own message component via `ctx.ui.setWidget` (or revisit the original overlay-takes-the-screen approach). Required to actually match Dan's screenshot. Coupled to whether we want pi-team to keep using pi's chat at all or own the UI.

8. **Roster + till-done styling polish** (deferred from 2026-04-27). Dan's footer has tier icons (⚡ orchestrator, ◆ lead), `⚡` next to the *currently-active* agent (confirmed by user — both `Orch` and `Setup Lead` had ⚡ when both were running), inline `$cost` per row, and `🧠 <context tokens>` for context-window usage. Till-done items grouped by tier with ellipsis-at-panel-width truncation. We have all the data — ~30 min of presentation.

9. **Plan immutability** (deferred from 2026-04-27 stress test). The `plan(...)` tool currently calls `tillDone.clear()` and re-seeds unconditionally, so when the orchestrator decides mid-run to "update the plan" by calling `plan` a second time, it nukes the in-flight till-done state (verified during Challenge 3 stress test: progress went from `[2/7]` back to `[0/7]`). Fix: in `plan.execute`, reject any second call if the till-done has any items. Return an error message that forces the orchestrator to work with the existing tasks: "plan() already registered. The plan is immutable for this user request — call mention(taskId=...) on the existing tasks." ~5 minutes of work; do after current run lands so we don't disturb in-flight state.
